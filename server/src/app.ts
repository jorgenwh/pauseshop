...
const createApp = (): express.Application => {
    ...
    console.log(`ANALYSIS_PROVIDER: "${process.env.ANALYSIS_PROVIDER || 'not set'}"`);
    ...
    if (currentProvider === 'openai') {
        ...
        console.log(`  API Key: ${process.env.REQUESTY_API_KEY ? 'Set (****)' : 'NOT SET'}`);
        console.log(`  Model: ${process.env.REQUESTY_MODEL || 'openai/gpt-4o-mini (default)'}`);
        console.log(`  Max Tokens: ${process.env.REQUESTY_MAX_TOKENS || '1000 (default)'}`);
        console.log(`  Site URL: ${process.env.REQUESTY_SITE_URL || 'not set'}`);
        console.log(`  Site Name: ${process.env.REQUESTY_SITE_NAME || 'not set'}`);
    }

    // Validate provider configuration
    const validation = AnalysisProviderFactory.validateProviderConfig();
    console.log(`Provider Config: ${validation.isValid ? '✅ Valid' : '❌ Invalid - ' + validation.error}`);
    console.log('=================================');
