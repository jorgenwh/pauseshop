    ...
    describe("executeAmazonSearch", () => {
        ...
        it("should successfully execute search and return HTML content", async () => {
            const mockHtmlContent =
                "<html><body>Amazon search results</body></html>";
            ...
            expect(result.searchUrl).toBe(
                "https://www.amazon.com/s?k=test+product",
            );
        ...
        it("should detect Amazon anti-bot protection", async () => {
            ...
            expect(result.error).toContain(
                "Amazon anti-bot protection detected",
            );
        });
        ...
        it("should apply custom configuration", async () => {
            const mockHtmlContent =
                "<html><body>Amazon search results with custom config</body></html>";
            ...
            const result = await executeAmazonSearch(
                mockSearchResult,
                customConfig,
            );
    ...
    describe("executeAmazonSearchBatch", () => {
        ...
        it("should execute batch searches successfully", async () => {
            const mockHtmlContent1 =
                "<html><body>Search results 1</body></html>";
            const mockHtmlContent2 =
                "<html><body>Search results 2</body></html>";
            ...
            expect(result.executionResults[0].htmlContent).toBe(
                mockHtmlContent1,
            );
            expect(result.executionResults[1].success).toBe(true);
            expect(result.executionResults[1].htmlContent).toBe(
                mockHtmlContent2,
            );
        });
        ...
        it("should filter out invalid search results", async () => {
            ...
            const result = await executeAmazonSearchBatch(
                batchWithInvalidResults,
            );
