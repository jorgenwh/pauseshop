...
describe("Amazon Search URL Construction", () => {
    describe("constructSingleAmazonSearch", () => {
        test("should construct valid URL for clothing product", () => {
            ...
            expect(result.searchUrl).toContain(
                "men+navy+blue+cotton+t-shirt+Nike",
            );
        ...
        test("should truncate very long search terms", () => {
            const longProduct: Product = {
                ...mockProducts[0],
                searchTerms:
                    "this is a very long search term that exceeds the maximum length limit and should be truncated to prevent URL issues and ensure proper functionality of the Amazon search system",
            };
    ...
    describe("constructAmazonSearchBatch", () => {
        ...
        test("should maintain product order in results", () => {
            ...
            result.searchResults.forEach((searchResult, index) => {
                expect(searchResult.originalProduct).toEqual(
                    mockProducts[index],
                );
            });
        });
    ...
    describe("Fallback behavior", () => {
        test("should fallback to product name when searchTerms is empty", () => {
            ...
            const result = constructSingleAmazonSearch(
                productWithoutSearchTerms,
            );
