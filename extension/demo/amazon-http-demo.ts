...
import { constructAmazonSearchBatch, constructSingleAmazonSearch } from '../src/scraper/amazon-search';
...
export async function runAmazonHttpDemo(): Promise<void> {
    ...
    try {
        ...
        sampleProducts.forEach((product, index) => {
            ...
            console.log(`   Brand: ${product.brand}, Color: ${product.primaryColor}`);
        ...
        console.log(`✅ URLs constructed: ${searchBatch.metadata.successfulSearches}/${searchBatch.metadata.totalProducts} successful`);
        console.log(`⏱️  Processing time: ${searchBatch.metadata.processingTime}ms\n`);
        ...
        console.log(`   Max Concurrent Requests: ${httpConfig.maxConcurrentRequests}`);
        ...
        console.log(`   User Agent Rotation: ${httpConfig.userAgentRotation}\n`);

        // Step 4: Execute Amazon search requests
        console.log('🌐 Executing Amazon search requests...');
        console.log('⚠️  Note: This will make actual HTTP requests to Amazon.com\n');
        ...
        console.log(`   Total Requests: ${executionResults.metadata.totalRequests}`);
        console.log(`   Successful: ${executionResults.metadata.successfulRequests}`);
        console.log(`   Failed: ${executionResults.metadata.failedRequests}`);
        console.log(`   Total Execution Time: ${executionResults.metadata.totalExecutionTime}ms`);
        console.log(`   Average Response Time: ${executionResults.metadata.averageResponseTime.toFixed(0)}ms\n`);

        // Display individual results
        console.log('📄 Individual Request Results:');
        executionResults.executionResults.forEach((result, index) => {
            console.log(`${index + 1}. Product: ${result.originalSearchResult.originalProduct.name}`);
            console.log(`   Success: ${result.success ? '✅' : '❌'}`);
            
            if (result.success) {
                console.log(`   Status Code: ${result.statusCode}`);
                console.log(`   Response Time: ${result.responseTime}ms`);
                console.log(`   HTML Size: ${result.htmlContent?.length || 0} characters`);
                console.log(`   Contains "Amazon": ${result.htmlContent?.includes('Amazon') ? 'Yes' : 'No'}`);
                console.log(`   Contains Products: ${result.htmlContent?.includes('data-component-type="s-search-result"') ? 'Yes' : 'No'}`);
            } else {
        ...
        if (searchBatch.searchResults.length > 0) {
            const singleResult = await executeAmazonSearch(searchBatch.searchResults[0], {
                timeoutMs: 10000,
                maxRetries: 1
            });

            console.log(`Single search result: ${singleResult.success ? 'Success' : 'Failed'}`);
            if (singleResult.success) {
                console.log(`HTML preview: ${singleResult.htmlContent?.substring(0, 200)}...`);
            } else {
...
export function runUrlConstructionDemo(): void {
    ...
    console.log(`Confidence: ${(singleSearchResult.confidence * 100).toFixed(1)}%\n`);
...
if (typeof window !== 'undefined') {
    ...
    console.log('- amazonHttpDemo.runUrlConstructionDemo() // Safe, no HTTP requests');
    console.log('- amazonHttpDemo.runAmazonHttpDemo() // Makes actual HTTP requests to Amazon');
