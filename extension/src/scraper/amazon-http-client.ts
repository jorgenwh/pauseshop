...
class AmazonHttpClient {
    ...
    private async executeAmazonSearchWithRetry(
        ...
        try {
            ...
            // Generate headers
            const headers = this.generateRealisticHeaders(searchResult.searchUrl);
            ...
            if (retryCount < this.config.maxRetries) {
                console.warn(`Amazon request failed (attempt ${retryCount + 1}/${this.config.maxRetries + 1}): ${errorMessage}`);
                
                // Exponential backoff with jitter
                const backoffDelay = (1000 * Math.pow(2, retryCount)) + (Math.random() * 1000);
                await new Promise(resolve => setTimeout(resolve, backoffDelay));

                return this.executeAmazonSearchWithRetry(searchResult, retryCount + 1);
            }
    ...
    private async processQueue(): Promise<void> {
        while (this.requestQueue.length > 0 && this.activeRequests < this.config.maxConcurrentRequests) {
            ...
            this.executeAmazonSearchWithRetry(queuedRequest.searchResult, queuedRequest.retryCount)
     ...
     */
    async executeAmazonSearch(searchResult: AmazonSearchResult): Promise<AmazonSearchExecutionResult> {
     ...
     */
    async executeAmazonSearchBatch(searchBatch: AmazonSearchBatch): Promise<AmazonSearchExecutionBatch> {
        const startTime = Date.now();
        ...
        try {
            ...
            const successfulRequests = executionResults.filter(r => r.success).length;
            const failedRequests = executionResults.length - successfulRequests;
            const totalResponseTime = executionResults.reduce((sum, r) => sum + r.responseTime, 0);
            const averageResponseTime = executionResults.length > 0 
